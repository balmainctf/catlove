...
Model.prototype.$__handleSave = function $__handleSave(options) {
  ...
  if (this.isNew) {
    ...
    return new Promise.ES6(function(resolve, reject) {
      ...
      _this.$__version(true, obj);
      _this.collection.insert(obj, options.safe, function (err, ret) {
        if (err) {
          _this.isNew = true;
      ...
      _this.$__reset();
      _this.isNew = false;
      _this.emit('isNew', false);
      // Make it possible to retry the insert
      _this.$__.inserting = true;
    });
  } else {
    ...
    return new Promise.ES6(function(resolve, reject) {
      if (delta) {
        ...
        var where = _this.$__where(delta[0]);
        ...
        _this.collection.update(where, delta[1], options.safe, function (err, ret) {
          if (err) {
            reject(err);
            return;
          }
...
Model.prototype.$__version = function (where, delta) {
  ...
  // fast here and force them to includes the versionKey but
 ...
 *
 *     // includes all properties except for `length`
...
function populate(model, docs, options, cb) {
  ...
  for (i = 0; i < len; i++) {
    ...
    var ids = utils.array.flatten(mod.ids, function(item) {
      // no need to includes undefined values in our query
